...
async function submitScore(name, score, isMultiplayer = false) {
  ...
  try {
    const { error } = await supabaseClient
      .from("leaderboard")
      .insert([
        { player_name: name, score: score, is_multiplayer: isMultiplayer },
      ]);
    if (error) {
...
async function initAudioSharing() {
  ...
  try {
    ...
    if (!localAudioStream || !localAudioStream.active) {
      console.log(
        "Requesting microphone access (attempt",
        audioRetryCount + 1,
        ")...",
      );
      try {
        ...
        if (permError.name === "NotAllowedError") {
          ...
          if (audioRetryCount < MAX_AUDIO_RETRIES) {
            ...
            console.log(
              `Retrying audio init in ${delayMs}ms (${audioRetryCount + 1}/${MAX_AUDIO_RETRIES})`,
            );
            audioRetryCount++;
            showAudioError(
              `Audio interrupted. Retrying (${audioRetryCount}/${MAX_AUDIO_RETRIES})...`,
            );
            ...
            showAudioError(
              "Audio initialization failed after multiple attempts.",
            );
            audioRetryCount = 0;
            return false;
          }
    ...
    audioPeerConnection.onicecandidate = (event) => {
      if (event.candidate) {
        ...
        if (multiplayerChannel) {
          ...
          console.error(
            "✗ Cannot send ICE candidate - multiplayerChannel is null",
          );
        }
      } else {
    ...
    audioPeerConnection.onicegatheringstatechange = () => {
      console.log(
        "ICE gathering state:",
        audioPeerConnection.iceGatheringState,
      );
    };
    ...
    audioPeerConnection.oniceconnectionstatechange = () => {
      console.log(
        ...
        audioPeerConnection.iceConnectionState,
      );
    ...
    audioPeerConnection.onconnectionstatechange = () => {
      ...
      if (state === "connected") {
        ...
        if (!isReconnecting && audioRetryCount < MAX_AUDIO_RETRIES) {
          ...
          const delayMs = calculateBackoffDelay(audioRetryCount);
          console.error(
            `Audio connection failed. Reconnecting in ${delayMs}ms (${audioRetryCount + 1}/${MAX_AUDIO_RETRIES})`,
          );
          audioRetryCount++;
          showAudioError(
            `Connection lost. Reconnecting (${audioRetryCount}/${MAX_AUDIO_RETRIES})...`,
          );
        ...
        setTimeout(() => {
          if (
            audioPeerConnection &&
            audioPeerConnection.connectionState === "disconnected"
          ) {
            console.log(
              "Connection still disconnected, triggering reconnection",
            );
            audioPeerConnection.dispatchEvent(
    ...
    audioConnectionTimeoutId = setTimeout(() => {
      if (
        ...
        console.error(
          "Audio connection timeout after " + AUDIO_CONNECTION_TIMEOUT + "ms",
        );
        ...
        if (audioRetryCount < MAX_AUDIO_RETRIES) {
          ...
          audioRetryCount++;
          showAudioError(
            `Connection timeout. Retrying (${audioRetryCount}/${MAX_AUDIO_RETRIES})...`,
          );
...
async function handleAudioIceCandidate(data) {
  ...
  if (!audioPeerConnection) {
    ...
    if (audioIceCandidatesQueue.length < MAX_ICE_QUEUE) {
      ...
      console.warn(
        "✗ ICE candidate queue full (" + MAX_ICE_QUEUE + "), dropping oldest",
      );
...
async function processIceCandidatesQueue() {
  ...
  console.log(
    "Processing ICE candidates queue:",
    audioIceCandidatesQueue.length,
  );
...
function validateAudioConnection() {
  ...
  const audioTracks = receivers.filter(
...
function startAudioStatsMonitoring() {
  // Audio stats monitoring disabled for performance
  // Quality monitoring is handled by startConnectionQualityMonitoring
}
  ...
  create() {
    ...
    if (this.textures.exists("poster")) {
      ...
      const scale = Math.min(
        width / this.posterImage.width,
        height / this.posterImage.height,
      );
      this.posterImage.setScale(isCompact ? scale * 0.92 : scale);
      this.posterImage.setAlpha(0);

      this.tweens.add({
        targets: this.posterImage,
        alpha: 1,
        duration: 800,
        ease: "Power2",
      });
    ...
    const promptText = this.add
      .text(width / 2, height - (isCompact ? 52 : 70), "TAP TO START", {
    ...
    soundHint.innerHTML =
      '<i class="fa-solid fa-volume-up" aria-hidden="true"></i><span> Enable sound for best experience</span>';
    ...
    getLeaderboard()
      .then((scores) => {
        ...
        if (
          this.scene &&
          this.scene.isActive("SplashScene") &&
          this.highScoreText &&
          this.highScoreText.active
        ) {
          const topScore = scores && scores.length > 0 ? scores[0].score : 0;
          this.highScoreText.setText(
            `BEAT THE CURRENT HIGH SCORE: ${topScore}`,
          );
        }
      })
      .catch((err) => {
        console.warn(
          "Leaderboard fetch failed (expected if local or offline):",
          err,
        );
      });
...
class ModeSelectScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.add
      .text(width / 2, height * 0.15, "SELECT MODE", {
        fontFamily: "Fredoka, sans-serif",
        fontSize: titleSize,
        color: "#ffffff",
        stroke: "#000000",
    ...
    const singleBg = this.add
      .rectangle(width / 2, singleY, btnWidth, btnHeight, 0x1a3a2a, 0.9)
      .setStrokeStyle(3, 0x00ff88, 1)
      .setInteractive({ useHandCursor: true });

    const singleText = this.add
      .text(width / 2 + 12, singleY - 12, "SINGLE PLAYER", {
        fontFamily: "Fredoka, sans-serif",
    ...
    Icons.drawUser(
      singleIcon,
      singleText.x - singleText.width / 2 - singleIconSize,
      singleY - 12,
      singleIconSize,
      0x00ff88,
      2,
    ...
    this.add
      .text(width / 2, singleY + 16, "Classic solo adventure", {
    ...
    const multiBg = this.add
      .rectangle(width / 2, multiY, btnWidth, btnHeight, 0x1a2a3a, 0.9)
      .setStrokeStyle(3, 0x00ccff, 1)
      .setInteractive({ useHandCursor: true });

    const multiText = this.add
      .text(width / 2 + 12, multiY - 12, "MULTIPLAYER", {
        fontFamily: "Fredoka, sans-serif",
        fontSize: isMobile ? "18px" : isCompact ? "22px" : "26px",
        color: "#00ccff",
      })
    ...
    Icons.drawUsers(
      multiIcon,
      multiText.x - multiText.width / 2 - multiIconSize,
      multiY - 12,
      multiIconSize,
      0x00ccff,
      2,
    ...
    this.add
      .text(width / 2, multiY + 16, "Team up with a friend!", {
        fontFamily: "Outfit, sans-serif",
        fontSize: isMobile ? "12px" : isCompact ? "14px" : "16px",
        color: "#aaccff",
      })
    ...
    if (!supabaseClient) {
      ...
      this.add
        .text(width / 2, multiY + 40, "(Requires online connection)", {
          fontFamily: "Outfit, sans-serif",
          fontSize: "11px",
          color: "#666666",
        })
...
class LobbyScene extends Phaser.Scene {
  ...
  create() {
    ...
    // Title
    this.add
      .text(width / 2, height * 0.1, "MULTIPLAYER LOBBY", {
        fontFamily: "Fredoka, sans-serif",
  ...
  createLobbyMenu() {
    ...
    const btnY = height * 0.4;

    // Create Room Button
    const createBg = this.add
      .rectangle(width / 2, btnY, btnWidth, btnHeight, 0x1a3a2a, 0.9)
      .setStrokeStyle(3, 0x00ff88, 1)
      .setInteractive({ useHandCursor: true });

    const createText = this.add
      .text(width / 2 + 14, btnY, "CREATE ROOM", {
        fontFamily: "Fredoka, sans-serif",
        fontSize: isMobile ? "18px" : isCompact ? "22px" : "26px",
        color: "#00ff88",
      })
    ...
    Icons.drawHome(
      createIcon,
      createText.x - createText.width / 2 - createIconSize,
      btnY,
      createIconSize,
      0x00ff88,
      2,
    ...
    const joinBg = this.add
      .rectangle(width / 2, joinY, btnWidth, btnHeight, 0x1a2a3a, 0.9)
      .setStrokeStyle(3, 0x00ccff, 1)
      .setInteractive({ useHandCursor: true });

    const joinText = this.add
      .text(width / 2 + 14, joinY, "JOIN ROOM", {
        fontFamily: "Fredoka, sans-serif",
        fontSize: isMobile ? "18px" : isCompact ? "22px" : "26px",
        color: "#00ccff",
      })
    ...
    Icons.drawLink(
      joinIcon,
      joinText.x - joinText.width / 2 - joinIconSize,
      joinY,
      joinIconSize,
      0x00ccff,
      2,
  ...
  async showCreateRoom() {
    ...
    const creatingText = this.add
      .text(width / 2, height * 0.4, "Creating room...", {
        fontFamily: "Fredoka, sans-serif",
        fontSize: "20px",
        color: "#ffffff",
      })
  ...
  showWaitingForPlayer() {
    ...
    // Room code display
    this.add
      .text(width / 2, height * 0.25, "ROOM CODE", {
        fontFamily: "Outfit, sans-serif",
        fontSize: "14px",
        color: "#888888",
      })
    ...
    const codeBox = this.add
      .rectangle(width / 2, height * 0.35, 200, 70, 0x000000, 0.6)
      .setStrokeStyle(3, 0x00ff88, 1);

    const codeText = this.add
      .text(width / 2, height * 0.35, roomCode, {
        fontFamily: "Fredoka, sans-serif",
    ...
    const copyLabel = this.add
      .text(width / 2, height * 0.42, "CLICK TO COPY", {
        fontFamily: "Outfit, sans-serif",
        fontSize: "10px",
        color: "#00ccff",
    ...
    const waitingText = this.add
      .text(width / 2, height * 0.5, "Waiting for player to join...", {
        fontFamily: "Outfit, sans-serif",
        fontSize: "16px",
        color: "#ffffff",
      })
    ...
    const shareText = this.add
      .text(width / 2, height * 0.6, "Share this code with your friend!", {
        fontFamily: "Outfit, sans-serif",
        fontSize: "14px",
        color: "#aaaaaa",
      })
    ...
    const cancelBtn = this.add
      .text(width / 2, height * 0.75, "CANCEL", {
        fontFamily: "Fredoka, sans-serif",
        fontSize: "18px",
        color: "#ff6666",
  ...
  showJoinRoom() {
    ...
    const labelText = this.add
      .text(width / 2, height * 0.3, "ENTER ROOM CODE", {
        fontFamily: "Outfit, sans-serif",
        fontSize: "14px",
        color: "#888888",
      })
    ...
    this.inputEl.style.cssText = `
            ...
            top: ${canvasRect.top + height * 0.4}px;
    ...
    // Cancel button
    const cancelBtn = this.add
      .text(width / 2, height * 0.78, "CANCEL", {
        fontFamily: "Outfit, sans-serif",
        fontSize: "16px",
        color: "#888888",
      })
...
class GameScene extends Phaser.Scene {
  ...
  create() {
    ...
    this._lastRenderedLives = -1; // Force initial heart render
    this._lastTargetScore = -1; // Force initial score render
    this._lastRemoteHudUpdate = 0;
    ...
    this.scale.on("resize", (gameSize) => {
      ...
      if (this.isMultiplayer) {
        console.log(
          "Ignoring resize during multiplayer to preserve connection",
        );
  ...
  calculateLayout() {
    ...
    this.rainDropCount = this.isCompact ? 25 : 50;
    this.rainLayerCount = this.isCompact ? 2 : 3;
  ...
  createEnvironment() {
    ...
    for (let l = 0; l < layers; l++) {
      ...
      for (
        ...
        g.fillTriangle(
          x + w * 0.1,
          this.topSafe - height * 0.3,
          x + w / 2,
          this.topSafe - height * 0.7,
          x + w * 0.9,
          this.topSafe - height * 0.3,
        );
        // Top tier
    ...
    for (let l = 0; l < waveLayers; l++) {
      const yBase =
        this.botSafe + 10 + l * ((height - this.botSafe) / waveLayers);
      ...
      const shimmerStep = this.isCompact ? 120 : 80;
      for (
        let x = 0;
        x < width;
        x += shimmerStep + Math.random() * shimmerStep
      ) {
        const y = yBase + Math.sin(x * freq + l) * amp;
    ...
    this.add
      .text(width / 2, this.roadY + this.roadHeight / 2, "ALMA BRIDGE ROAD", {
        fontFamily: "Outfit, sans-serif",
    ...
    const topText = this.add
      .text(
        width / 2 + 12,
        this.topSafe - topTextOffset,
        "OPEN SPACE PRESERVE",
        { ...fancyStyle, color: "#44dd66" },
      )
      .setOrigin(0.5);
    const topIcon = this.add.graphics();
    Icons.drawMapPin(
      topIcon,
      topText.x - topText.width / 2 - (this.isCompact ? 12 : 18),
      this.topSafe - topTextOffset - 1,
      this.isCompact ? 14 : 18,
      0x44dd66,
    );

    const botTextOffset = this.isCompact ? 18 : 25;
    const botText = this.add
      .text(
        width / 2 + 12,
        this.botSafe + botTextOffset,
        "LEXINGTON RESERVOIR",
        { ...fancyStyle, color: "#44aadd" },
      )
      .setOrigin(0.5);
    const botIcon = this.add.graphics();
    Icons.drawMapPin(
      botIcon,
      botText.x - botText.width / 2 - (this.isCompact ? 12 : 18),
      this.botSafe + botTextOffset - 1,
      this.isCompact ? 14 : 18,
      0x44aadd,
    );
    ...
    if (this.textures.exists("newtXing")) {
      this.add
        .image(signOffset, this.topSafe - topTextOffset, "newtXing")
        .setDisplaySize(signSize, signSize);
      this.add
        .image(width - signOffset, this.botSafe - topTextOffset, "newtXing")
        .setDisplaySize(signSize, signSize);
    }
  }
  ...
  async initMultiplayerAudio() {
    ...
    console.log(
      "Existing audio connection:",
      audioPeerConnection?.connectionState || "none",
    );
    console.log("═══════════════════════════════════════");
    ...
    if (
      ...
      console.log(
        "✓ Audio already connected/connecting, skipping re-init. Just recreating mute button.",
      );
    ...
    if (!multiplayerChannel || multiplayerChannel.state !== "joined") {
      ...
      let waited = 0;
      while (
        (!multiplayerChannel || multiplayerChannel.state !== "joined") &&
        waited < 5000
      ) {
        await new Promise((resolve) => setTimeout(resolve, 500));
        waited += 500;
      ...
      if (!multiplayerChannel || multiplayerChannel.state !== "joined") {
        console.error(
          "✗ Channel not ready after waiting. Cannot initialize audio.",
        );
        return;
      }
    }
  ...
  createMuteButton() {
    ...
    // Button background
    this.muteBtnBg = this.add
      .rectangle(muteBtnX, muteBtnY, muteBtnSize, muteBtnSize, 0x000000, 0.8)
    ...
    this.muteTooltip = this.add
      .text(muteBtnX, muteBtnY + muteBtnSize / 2 + 10, "Mute", {
        fontFamily: "Outfit, sans-serif",
        fontSize: "10px",
        color: "#ffffff",
      })
  ...
  updateMuteIcon() {
    ...
    if (this.muteTooltip && this.muteTooltip.active)
      this.muteTooltip.setText(tooltipText);
  ...
  createRemotePlayer() {
    ...
    const remoteStats =
      CHARACTER_STATS[remoteCharacter] || CHARACTER_STATS["male"];
    this.remotePlayer.carryCapacity = remoteStats.carryCapacity;
    this.remotePlayer.carried = [];

    console.log(
      "✓ Remote player created at position:",
      startX,
      this.botSafe + 60,
    );
  ...
  setupMultiplayerSync() {
    ...
    if (multiplayerChannel) {
      console.log(
        "Cleaning up old channel (state:",
        multiplayerChannel.state,
        ")",
      );
      try {
        supabaseClient.removeChannel(multiplayerChannel);
    ...
    multiplayerChannel.on("broadcast", { event: "player_ready" }, (payload) => {
      ...
      if (payload.payload.playerId !== playerId) {
        ...
        if (!remoteCharacter && payload.payload.character) {
          ...
          console.log(
            "Updated remoteCharacter from ready announcement:",
            remoteCharacter,
          );
  ...
  handleRemotePlayerUpdate(data) {
    ...
    // Throttle HUD updates from remote sync to max ~5Hz (every 200ms)
    const now = Date.now();
    if (!this._lastRemoteHudUpdate || now - this._lastRemoteHudUpdate > 200) {
      this._lastRemoteHudUpdate = now;
      this.updateHUD();
    }
  }
  ...
  syncNewts(newtsData) {
      ...
      if (existing) {
        ...
        if (!nData.isCarried) {
          ...
          if (nData.carriedBy === playerId) {
            ...
            if (
              this.remotePlayer &&
              !this.remotePlayer.carried.includes(existing)
            ) {
    ...
    if (this.remotePlayer && this.remotePlayer.carried) {
      this.remotePlayer.carried = this.remotePlayer.carried.filter(
        (n) => n && n.active && n.isCarried,
      );
    }
  }
  ...
  handleNewtSave(data) {
    ...
    if (data.newtId) {
      const savedNewt = this.newts
        .getChildren()
        .find((n) => n.newtId === data.newtId);
      if (savedNewt) {
        if (this.remotePlayer && this.remotePlayer.carried) {
          this.remotePlayer.carried = this.remotePlayer.carried.filter(
            (n) => n !== savedNewt,
    ...
    if (data.correct) {
      this.teamScore += 100;
      this.saved++;
      if (this.cache.audio.exists("sfx_saved"))
        this.sound.play("sfx_saved", { volume: 0.6 });
  ...
  requestPlayerHit() {
    if (!multiplayerChannel || !this.isMultiplayer || isHost || this.gameOver)
      return;
    const now = Date.now();
  ...
  createImpactExplosion(x, y) {
    ...
    for (let i = 0; i < 8; i++) {
      ...
      const particle = this.add.circle(
        ...
        centerY,
        size,
        Phaser.Display.Color.GetColor(255, Phaser.Math.Between(50, 150), 0),
        0.9,
      );
  ...
  handlePartnerDisconnect() {
    ...
    const disconnectText = this.add
      .text(width / 2, height / 2 - 30, "Partner Disconnected", {
        fontFamily: "Fredoka, sans-serif",
        fontSize: "28px",
        color: "#ff6666",
        stroke: "#000000",
    ...
    const subText = this.add
      .text(width / 2, height / 2 + 10, "Game ending...", {
        fontFamily: "Outfit, sans-serif",
        fontSize: "16px",
        color: "#ffffff",
      })
  ...
  drawMalePlayer(g, isPlayer2 = false) {
    ...
    g.fillEllipse(0, -40, 26, 14); // Main cap
    g.fillStyle(capDarkColor);
    g.fillRect(-13, -42, 26, 6); // Cap brim
  }
  ...
  createHUD() {
    ...
    const baseFontSize =
      this.isMobileDevice && this.isMultiplayer
        ? "14px"
        : this.isCompact
          ? "16px"
          : "20px";
    ...
    this.scoreText = this.add
      .text(
        this.scale.width - padding - 6,
        padding + (this.isCompact ? 12 : 18),
        "",
        {
          fontFamily: "Fredoka, sans-serif",
    ...
    this.add
      .text(this.scale.width - padding - 6, padding - 2, "SCORE", {
        fontFamily: "Outfit, sans-serif",
    ...
    this.carryText = this.add
      .text(this.scale.width / 2, padding, "", {
        ...style,
        color: "#00ffff",
        shadow: {
          offsetX: 2,
    ...
    const statsFontSize = this.isMobileDevice
      ? "16px"
      : this.isCompact
        ? "18px"
        : "22px";
    this.statsText = this.add
      .text(padding + 2, this.scale.height - padding - 2, "", {
        fontFamily: "Fredoka, sans-serif",
    ...
    if (this.isMultiplayer && roomCode) {
      const roomFontSize = this.isMobileDevice
        ? "10px"
        : this.isCompact
          ? "12px"
          : "14px";
      this.roomCodeText = this.add
        .text(padding, this.isCompact ? 50 : 60, `ROOM: ${roomCode}`, {
          fontFamily: "Outfit, sans-serif",
  ...
  updateHUD() {
    ...
    // Only recreate heart icons when lives actually change
    if (this._lastRenderedLives !== this.lives) {
      this._lastRenderedLives = this.lives;
    ...
    if (this.isMultiplayer) {
      ...
      if (this.isMobileDevice) {
        ...
        this.carryText.setText(
          `P1: newts x ${p1Count} | P2: newts x ${p2Count}`,
        );
      }
    } else {
      ...
      this.carryText.setText(
        `Carrying ${carryCount} of ${maxCapacity} Newt${maxCapacity > 1 ? "s" : ""}`,
      );
    }
  ...
  updateDifficulty() {
    ...
    if (scoreToUse >= GAME_CONFIG.DIFFICULTY_THRESHOLD) {
      ...
      const newDelay = Math.max(
        600,
        GAME_CONFIG.CAR_SPAWN_RATE / this.difficulty,
      );
  ...
  createControls() {
    ...
    this.joyBase = this.add
      .circle(0, 0, joyBaseSize, 0xffffff, 0.15)
      .setStrokeStyle(2, 0x00ffff, 0.5)
      .setVisible(false)
      .setDepth(500);
    this.joyThumb = this.add
      .circle(0, 0, joyThumbSize, 0x00ffff, 0.4)
      .setVisible(false)
      .setDepth(501);
    ...
    this.input.on("pointermove", (p) => {
      ...
      if (dist > 0) {
        ...
        this.joyThumb.setPosition(
          this.inputData.sx + dx * (clamped / dist),
          this.inputData.sy + dy * (clamped / dist),
        );
      }
    });
  ...
  updateCars(delta) {
    ...
    for (let ci = numCars - 1; ci >= 0; ci--) {
      ...
      const targetSpeed =
        car.type === "motorbike"
          ? GAME_CONFIG.CAR_MAX_SPEED * 1.4 * this.difficulty * sign
          : car.type === "truck"
            ? GAME_CONFIG.CAR_MIN_SPEED * 0.8 * this.difficulty * sign
            : GAME_CONFIG.CAR_MIN_SPEED * 1.2 * this.difficulty * sign;
      ...
      for (let oi = 0; oi < numCars; oi++) {
        const other = cars[oi];
        if (ci === oi || !other || !other.active) continue;

        // Same lane check
        if (Math.abs(car.y - other.y) < 10) {
          const dx = other.x - car.x;
          if (dir === 1 && dx > 0 && dx < 200) {
            if (dx < minDist) {
              minDist = dx;
              carAhead = other;
            }
          } else if (dir === -1 && dx < 0 && dx > -200) {
            const dist = -dx;
            if (dist < minDist) {
              minDist = dist;
              carAhead = other;
      ...
      if (carAhead) {
        ...
        if (
          !car.isChangingLane &&
          minDist < 100 &&
          Math.abs(car.speed) < Math.abs(targetSpeed) * 0.8
        ) {
          this.tryOvertake(car, cars, dir);
        }
      }

      if (dir === 1 && car.x > screenW + 200) car.destroy();
  ...
  interpolateCars(delta) {
    ...
    for (let i = cars.length - 1; i >= 0; i--) {
      ...
      if (car.x > screenW + 200 || car.x < -200) {
        car.destroy();
      }
    }
  }
  ...
  tryOvertake(car, allCars, dir) {
    const laneIndex = Math.round(
      (car.y - this.roadY - this.laneHeight / 2) / this.laneHeight,
    );
    const candidates = [];
    ...
    for (const targetLane of candidates) {
      const targetY =
        this.roadY + targetLane * this.laneHeight + this.laneHeight / 2;
      let safe = true;
  ...
  spawnCar() {
    ...
    const existingCars = this.cars.getChildren();
    const screenW = this.scale.width;
    for (let i = 0; i < existingCars.length; i++) {
      const c = existingCars[i];
      if (Math.abs(c.y - y) < 10) {
        if (dir === 1 && c.x < -150 + safeDistance) {
          safeToSpawn = false;
          break;
        }
        if (dir === -1 && c.x > screenW + 150 - safeDistance) {
          safeToSpawn = false;
          break;
        }
      }
    ...
    const g = this.add.graphics();
    const colors = [
      0xe74c3c, 0x3498db, 0x2ecc71, 0x9b59b6, 0xf39c12, 0x1abc9c, 0xbdc3c7,
      0x34495e,
    ];
    ...
    container.lane = lane;
    container.carId =
      "car_" + Date.now() + "_" + Math.random().toString(36).substring(2, 8);
  ...
  spawnNewt() {
    ...
    if (this.textures.exists("newt")) {
      ...
      newt.isCarried = false;
      newt.newtId =
        "newt_" + Date.now() + "_" + Math.random().toString(36).substring(2, 8);
      newt.rotation = newt.dir === 1 ? Math.PI / 2 : -Math.PI / 2;
      this.newts.add(newt);
    }
  ...
  updateNewts(delta) {
    const children = this.newts.getChildren();
    const len = children.length;
    const dt = delta / 1000;
    const newtSpeed = GAME_CONFIG.NEWT_SPEED;
    const botLimit = this.botSafe + 30;
    const topLimit = this.topSafe - 30;
    // Cache shared sin values (same for all newts in a frame)
    const wiggle = this.isCompact ? 0 : Math.sin(this.time.now * 0.01) * 0.15;
    const carriedWiggle = this.isCompact
      ? 0
      : Math.sin(this.time.now * 0.008) * 0.2;
    const halfPi = Math.PI / 2;

    for (let i = len - 1; i >= 0; i--) {
      const newt = children[i];
      if (!newt || !newt.active) continue;

      if (!newt.isCarried) {
        newt.y += newt.dir * newtSpeed * dt;
        newt.rotation = (newt.dir === 1 ? halfPi : -halfPi) + wiggle;
        if (
          (newt.dir === 1 && newt.y > botLimit) ||
          (newt.dir === -1 && newt.y < topLimit)
        ) {
          newt.destroy();
        }
  ...
  checkCollisions() {
    ...
    const cars = this.cars.getChildren();
    const newts = this.newts.getChildren();
    const px = this.player.x;
    const py = this.player.y;
    const isInvincible = this.player.invincible;
    const checkNewtCar = !this.isMultiplayer || isHost;
    let playerHit = false;
    ...
    for (let ci = cars.length - 1; ci >= 0; ci--) {
      ...
      if (
        !isInvincible &&
        !playerHit &&
        Math.abs(px - car.x) < car.w / 2 &&
        Math.abs(py - car.y) < car.h / 2
      ) {
        playerHit = true;
        if (!this.isMultiplayer || isHost) {
          this.hitPlayer();
      ...
      // Newt-car collision (only host)
      if (checkNewtCar) {
        for (let ni = newts.length - 1; ni >= 0; ni--) {
          const newt = newts[ni];
          if (!newt || !newt.active || newt.isCarried) continue;
          if (
            Math.abs(newt.x - car.x) < car.w / 2 &&
            Math.abs(newt.y - car.y) < car.h / 2
          ) {
    ...
    // Newt pickup check (use squared distance to avoid sqrt)
    const pickupDistSq = 50 * 50;
    for (let ni = newts.length - 1; ni >= 0; ni--) {
      const newt = newts[ni];
      if (!newt || !newt.active) continue;
      if (
        !newt.isCarried &&
        !newt.carriedBy &&
        this.player.carried.length < this.player.carryCapacity
      ) {
        const dx = px - newt.x;
        const dy = py - newt.y;
    ...
    if (this.player.carried.length > 0) {
      ...
      if (inForest || inLake) {
        this.player.carried.forEach((newt) => {
          const correct =
            (newt.dest === "FOREST" && inForest) ||
            (newt.dest === "LAKE" && inLake);
          if (correct) {
            this.saved++;
            this.streak++;
  ...
  showAchievement(title, subtitle, iconClass = "fa-award") {
    ...
    for (let i = 0; i < 4; i++) {
      ...
      const spark = this.add
        .star(sparkX, sparkY, 4, 3, 6, 0xffcc00)
        .setDepth(200)
        .setAlpha(0);
  ...
  createPickupEffect(x, y) {
    ...
    const flash = this.add
      .circle(this.player.x, this.player.y, 50, 0x00ffff, 0.3)
      .setDepth(49);
    this.tweens.add({
      targets: flash,
      scale: 1.5,
  ...
  showFloatingText(x, y, message, color, isLarge = false) {
    ...
    const text = this.add
      .text(x, y, message, {
        ...
        shadow: isLarge
          ? { offsetX: 2, offsetY: 2, color: "#000000", blur: 4, fill: true }
          : null,
      })
      .setOrigin(0.5)
      .setDepth(150);
  ...
  animateScoreChange(targetScore) {
    ...
    if (targetScore === this._lastTargetScore) return;
    this._lastTargetScore = targetScore;
    ...
    this.scoreTween = this.tweens.add({
      ...
      ease: "Power2",
      onUpdate: () => {
        const rounded = Math.round(this.displayedScore);
        if (rounded !== lastRendered) {
          lastRendered = rounded;
          this.scoreText.setText(`${rounded}`);
        }
      },
      onComplete: () => {
  ...
  async showGameOver() {
    ...
    this.add
      ...
      .setDepth(300);
    this.add
      .text(width / 2, height * 0.08, "GAME OVER", {
        fontFamily: "Fredoka, sans-serif",
        fontSize: "44px",
        color: "#ff3366",
        fontStyle: "bold",
    ...
    this.add
      .text(width / 2, height * 0.16, `${scoreLabel}: ${finalScore}`, {
        fontFamily: "Fredoka, sans-serif",
        fontSize: "26px",
        color: "#ffffff",
      })
    ...
    const summaryBoxY =
      summaryTitleY + (isCompact ? 16 : 20) + summaryBoxHeight / 2;

    const summaryBg = this.add.graphics().setDepth(301);
    summaryBg.fillStyle(0x000000, 0.6);
    summaryBg.fillRoundedRect(
      width / 2 - summaryBoxWidth / 2,
      summaryBoxY - summaryBoxHeight / 2,
      summaryBoxWidth,
      summaryBoxHeight,
      12,
    );
    summaryBg.lineStyle(2, this.isMultiplayer ? 0x00ccff : 0x00ffff, 0.6);
    summaryBg.strokeRoundedRect(
      width / 2 - summaryBoxWidth / 2,
      summaryBoxY - summaryBoxHeight / 2,
      summaryBoxWidth,
      summaryBoxHeight,
    ...
    this.add
      .text(
        width / 2 - summaryBoxWidth / 2 + summaryPadX,
        summaryBoxY - summaryBoxHeight / 2 + summaryPadY,
        labelText,
        {
          fontFamily: "Outfit, sans-serif",
    ...
    const nextSectionY =
      summaryBoxY + summaryBoxHeight / 2 + (isCompact ? 18 : 24);
    ...
    if (supabaseClient) {
      ...
      const promptText = this.isMultiplayer
        ? "Enter your name (team submission):"
        : "Enter your name:";
      this.add
        .text(width / 2, namePromptY, promptText, {
          fontFamily: "Outfit, sans-serif",
          fontSize: "16px",
          color: "#aaaaaa",
        })
      ...
      const initialBtnText = this.isMultiplayer
        ? "Enter your name"
        : "SUBMIT SCORE";
      const initialBtnColor = this.isMultiplayer ? "#888888" : "#00ff00";
      const submitBtnText = this.add
        .text(width / 2 + 15, submitY, initialBtnText, {
          fontFamily: "Fredoka, sans-serif",
          fontSize: "20px",
          color: initialBtnColor,
          backgroundColor: "#222",
          padding: { left: 45, right: 18, top: 8, bottom: 8 },
        })
        .setOrigin(0.5)
        .setDepth(301);
      ...
      Icons.drawSend(
        submitIcon,
        submitBtnText.x - submitBtnText.width / 2 + 22,
        submitY,
        18,
        this.isMultiplayer ? 0x888888 : 0x00ff00,
      );
      ...
      this.add
        .text(width / 2, nextSectionY, "(Leaderboard not configured)", {
          fontFamily: "Outfit, sans-serif",
          fontSize: "14px",
          color: "#555",
        })
    ...
    const volunteerY = Math.min(
      height * 0.88,
      Math.max(desiredVolunteerY, minVolunteerY),
    );
    const volunteerBg = this.add
      .rectangle(width / 2, volunteerY, width * 0.85, 60, 0x004422, 0.9)
      .setStrokeStyle(2, 0x00ff88)
      .setOrigin(0.5)
      .setDepth(301);
    this.add
      .text(width / 2, volunteerY - 10, "Want to help real newts?", {
        fontFamily: "Fredoka, sans-serif",
        fontSize: "16px",
        color: "#ffffff",
      })
      .setOrigin(0.5)
      .setDepth(302);
    const volunteerLink = this.add
      .text(
        width / 2 + 10,
        volunteerY + 12,
        "Volunteer at bioblitz.club/newts",
        {
          fontFamily: "Fredoka, sans-serif",
          fontSize: "18px",
          color: "#00ff88",
    ...
    Icons.drawExternalLink(
      volunteerIcon,
      volunteerLink.x - volunteerLink.width / 2 - 18,
      volunteerY + 12,
      16,
      0x00ff88,
    );
    ...
    const retryBtnText = this.add
      .text(width / 2 + 15, height * 0.92, "TRY AGAIN", {
        fontFamily: "Fredoka, sans-serif",
        fontSize: "24px",
        color: "#00ffff",
        backgroundColor: "#222",
        padding: { left: 45, right: 22, top: 10, bottom: 10 },
      })
      .setOrigin(0.5)
      .setDepth(301)
      .setInteractive({ useHandCursor: true });
    ...
    Icons.drawRefresh(
      retryIcon,
      retryBtnText.x - retryBtnText.width / 2 + 22,
      height * 0.92,
      22,
      0x00ffff,
    );
  ...
  async showLeaderboard() {
    ...
    this.add
      .text(width / 2 + 10, startY, "TOP SCORES", {
        fontFamily: "Fredoka, sans-serif",
        fontSize: "18px",
        color: "#ffcc00",
      })
    ...
    if (scores.length === 0) {
      this.add
        .text(width / 2, startY + 30, "Be the first to set a high score!", {
          fontFamily: "Outfit, sans-serif",
          fontSize: "14px",
          color: "#666",
        })
...
class CharacterSelectScene extends Phaser.Scene {
  ...
  create() {
    ...
    const maleBox = this.add
      .rectangle(maleX, charY, boxWidth, boxHeight, 0x000000, 0.3)
      .setStrokeStyle(3, 0x00ffff, 1)
      .setInteractive({ useHandCursor: true });

    // Male glow effect (hidden initially)
    const maleGlow = this.add.ellipse(
      maleX,
      charY,
      boxWidth + 20,
      boxHeight + 20,
    ...
    this.add
      .text(maleX, charY + labelOffset, "VOLUNTEER A", {
        fontFamily: "Fredoka, sans-serif",
        fontSize: labelSize,
        color: "#00ffff",
        stroke: "#000000",
    ...
    const maleStatsText = this.add
      .text(maleX + 10, charY + statsOffset, "FAST | Carries 1", statsStyle)
      .setOrigin(0.5)
      .setColor("#8feaff")
    ...
    const maleIconX =
      maleStatsText.x - maleStatsText.width / 2 - (isMobile ? 12 : 14);
    ...
    const femaleBox = this.add
      .rectangle(femaleX, charY, boxWidth, boxHeight, 0x000000, 0.3)
      .setStrokeStyle(3, 0xff00ff, 1)
      .setInteractive({ useHandCursor: true });

    // Female glow effect (hidden initially)
    const femaleGlow = this.add.ellipse(
      femaleX,
      charY,
      boxWidth + 20,
      boxHeight + 20,
    ...
    // Female label
    this.add
      .text(femaleX, charY + labelOffset, "VOLUNTEER B", {
        fontFamily: "Fredoka, sans-serif",
        fontSize: labelSize,
        color: "#ff00ff",
        stroke: "#000000",
    ...
    // Female stats
    const femaleStatsText = this.add
      .text(femaleX + 10, charY + statsOffset, "STEADY | Carries 2", statsStyle)
      .setOrigin(0.5)
      .setColor("#ffb6de")
      .setShadow(0, 2, "#000000", 4, true, true);
    ...
    const femaleIconX =
      femaleStatsText.x - femaleStatsText.width / 2 - (isMobile ? 12 : 14);
    ...
    const updateSelection = (selected) => {
      ...
      selectIndicator.lineStyle(
        4,
        selected === "male" ? 0x00ffff : 0xff00ff,
        1,
      );
      const x = selected === "male" ? maleX : femaleX;
      const selBoxW = boxWidth + 10;
      const selBoxH = boxHeight + 10;
      selectIndicator.strokeRoundedRect(
        x - selBoxW / 2,
        charY - selBoxH / 2,
        selBoxW,
        selBoxH,
      ...
      // Update box styles
      maleBox.setStrokeStyle(
        selected === "male" ? 4 : 2,
        0x00ffff,
        selected === "male" ? 1 : 0.5,
      );
      femaleBox.setStrokeStyle(
        selected === "female" ? 4 : 2,
        0xff00ff,
        selected === "female" ? 1 : 0.5,
      );
    ...
    if (isMobile) {
      this.add
        .text(width / 2, charY + labelOffset + 35, "Tap to select", {
          fontFamily: "Outfit, sans-serif",
          fontSize: "12px",
          color: "#888888",
        })
    ...
    const startBtn = this.add
      .text(width / 2, height * (isMobile ? 0.82 : 0.85), "START GAME", {
        fontFamily: "Fredoka, sans-serif",
        fontSize: btnSize,
        color: "#000000",
    ...
    this.tweens.add({
      ...
      onUpdate: (tween, target) => {
        ...
        fillBar.fillRoundedRect(
          x + 2,
          y + 1,
          Math.max(0, fillWidth * target.progress - 4),
          height / 2.5,
          height / 5,
        );
      },
